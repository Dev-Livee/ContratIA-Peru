...
import {
  FiBarChart2, FiFolder, FiHome, FiLogOut, FiMenu, FiPlusCircle,
  FiSearch, FiSettings, FiUser,
} from 'react-icons/fi';
import { useAuth } from '@/context/AuthContext';

const NAV = [
  { label: 'Dashboard', path: '/entidad/dashboard', icon: FiHome },
  { label: 'Mis proyectos', path: '/entidad/proyectos', icon: FiFolder },
  { label: 'Nuevo proyecto', path: '/entidad/proyectos/nuevo', icon: FiPlusCircle },
    ...
    <VStack spacing={1} align="stretch" p={3} flex={1}>
      {NAV.map(item => {
        const isActive = location.pathname === item.path ||
          (item.path !== '/entidad/dashboard' && location.pathname.startsWith(item.path));
        return (
...
export default function EntidadLayout() {
  ...
  return (
    <Flex direction="column" minH="100vh">
      ...
      >
        <Button display={{ base: 'flex', md: 'none' }} variant="ghost" mr={3}
          aria-label="Menú" onClick={onOpen} p={2}>
